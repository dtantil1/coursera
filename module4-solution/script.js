...
*/
import { speak as speakHello, speakSimple as simpleHello } from './SpeakHello.js';
import { speak as speakGoodBye, speakSimple as simpleBye } from './SpeakGoodBye.js';
...
let looper = names.map(hail)
//print out array
for(let i = 0; i < looper.length; i++){console.log(looper[i])}
...
greetings = names.reduce((accumulator, currentValue)=>{
    if (currentValue.charAt(0).toLowerCase()==="j"){
      bye.push(simpleBye(currentValue));
    }
    else{
      hello.push(simpleHello(currentValue));
    }
    return greetings;
  }, greetings);
...
for(let i = 0; i < bye.length; i++){console.log(bye[i])}
